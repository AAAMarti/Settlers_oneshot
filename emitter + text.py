...
for index, row in df.iterrows():
    ...
    if prev_original_turn_number == row['original_turn_number']:
        ...
        if row['dialogue_act'] in dialogue_act_classes or len(concatenated_dialogue_act) == 1:
            ...
            formatted_lines.append(concatenated_text.strip())
            concatenated_text = str(row['emitter']) + ": " + str(row['text'])  # Convert to string
            concatenated_dialogue_act = {row['dialogue_act']}
    else:
        # Append the last concatenated text
...
output_file_path = r'D:\Chatbots\STAC\emitter_text333.txt'
